...
public class ShardPlugin implements Interceptor {
    ...
    public static final String configLocation = "configLocation";// shard-config.xml

    // ConcurrentHashMap<mapperId,needParse>
    private static final ConcurrentHashMap<String/* mapperId */, Boolean> cache =
    ...
    public void setProperties(Properties properties) {
        // 解析配置文件
        String configLocation = properties.getProperty(ShardPlugin.configLocation, null);
        ...
        try {//try-with-resources
            input = Resources.getResourceAsStream(configLocation);
            parser.parse(input);
        }
        catch (IOException e) {
    ...
    private boolean isShouldParse(String mapperId) {
        ...
        if (!mapperId.endsWith("!selectKey")) {
            ...
            ShardbatisConfig configHolder = ShardbatisConfig.getInstance();
